...
public class AsWithMissingMap : NonValidatingSpecBase
{
    ...
    public void Should_report_missing_map() => new Action(AssertConfigurationIsValid).ShouldThrow<InvalidOperationException>().Message.ShouldBe(
        "Missing map from Morphy.UnitTests.MappingInheritance.AsWithMissingMap+TModel to Morphy.UnitTests.MappingInheritance.AsWithMissingMap+TConcrete. Create using CreateMap<TModel, TConcrete>.");
}
public class AsShouldWorkOnlyWithDerivedTypesWithGenerics : MorphySpecBase
{
    class Source<T>
    {
    }
...
public class AsWithGenerics : MorphySpecBase
{
