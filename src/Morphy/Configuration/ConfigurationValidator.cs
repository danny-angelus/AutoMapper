    ...
    public void AssertConfigurationIsValid(TypeMap[] typeMaps)
    {
        ...
        var badTypeMaps =
            (from typeMap in typeMaps
             ...
             select new MorphyConfigurationException.TypeMapConfigErrors(typeMap, unmappedPropertyNames, canConstruct)).ToArray();
        if (badTypeMaps.Length > 0)
        {
            configExceptions.Add(new MorphyConfigurationException(badTypeMaps));
        }
        ...
        void DryRunTypeMap(TypePair types, TypeMap typeMap, MemberMap memberMap)
        {
            ...
            else
            {
                ...
                if (mapperToUse == null)
                {
                    configExceptions.Add(new MorphyConfigurationException(memberMap.TypeMap.Types) { MemberMap = memberMap });
                    return;
                }
