...
public sealed class TypeMap
{
    ...
    public string CheckRecord() => ConstructorMap?.Ctor is ConstructorInfo ctor && ctor.IsFamily && ctor.Has<CompilerGeneratedAttribute>() ?
        " When mapping to records, consider using only public constructors. See https://docs.Morphy.io/en/latest/Construction.html." : null;
    ...
    public void CheckProjection()
    {
        if (Projection)
        {
            throw new MorphyConfigurationException("CreateProjection works with ProjectTo, not with Map.", MissingMapException(Types));
        }
    }
    ...
    public void Seal(IGlobalConfiguration configuration)
    {
        ...
        {
            throw new MorphyMappingException("Error creating mapping strategy.", e, this);
        }
