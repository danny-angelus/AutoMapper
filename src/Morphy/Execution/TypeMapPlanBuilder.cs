...
public ref struct TypeMapPlanBuilder(IGlobalConfiguration configuration, TypeMap typeMap)
{
    ...
    private static MorphyMappingException MemberMappingError(Exception innerException, MemberMap memberMap) =>
    ...
    private void AddPathMaps(List<Expression> actions)
    {
        ...
        foreach (var pathMap in pathMaps)
        {
            ...
            catch (Exception e) when (e is not MorphyConfigurationException)
            {
                throw new MorphyMappingException("Error building path mapping strategy.", e, pathMap);
            }
        }
    }
    ...
    private void AddPropertyMaps(List<Expression> actions)
    {
        ...
        foreach (var propertyMap in propertyMaps)
        {
            ...
            {
                throw new MorphyMappingException("Error building member mapping strategy.", e, propertyMap);
            }
        }
    }
    ...
    private Expression CreateNewDestinationFunc() => _typeMap switch
    {
        ...
        { DestinationType: { IsInterface: true } interfaceType } => Throw(
            Constant(new MorphyMappingException("Cannot create interface " + interfaceType, null, _typeMap)),
            interfaceType),
    ...
    private Expression ConstructorMapping(ConstructorMap constructorMap)
    {
        ...
        foreach (var parameter in constructorMap.CtorParams)
        {
            ...
            {
                throw new MorphyMappingException("Error building constructor parameter mapping strategy.", e, parameter);
            }
        }
...
public class ValueConverter : ValueResolverConfig, IValueResolver
{
        ...
        Expression _, Expression destinationMember)
    {
        ...
        MorphyConfigurationException BuildExceptionMessage()
            => new(
